...
bool Manager::Write(EnumVal* id, RecordVal* columns_arg)
	...
	for ( list<Filter*>::iterator i = stream->filters.begin();
		...
		// Policy hooks may alter the log record or veto the
		// logging. Optimization: when no hook handlers are
		// defined, don't even invoke.
		if ( filter->policy && filter->policy->HasBodies() )
			{
